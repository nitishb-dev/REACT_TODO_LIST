...
const API_URL = "https://omh0mbb5td.execute-api.ap-south-1.amazonaws.com/"; // Replace with your API Gateway URL
...
const Todo = () => {
  ...
  const editTodo = async (id, updatedText, updatedStartDate, updatedEndDate) => {
    const updatedTask = { text: updatedText, startDate: updatedStartDate, endDate: updatedEndDate };
    ...
    <div className="min-h-screen flex flex-col items-center justify-center transition-all duration-300">
      ...
      <div className="shadow-lg rounded-2xl p-6 w-full max-w-md transition-all">
        ...
        <div className="flex flex-col space-y-2">
          ...
          <label htmlFor="endDate">Ending Date</label>
          <input id="endDate" ref={endDateRef} className="p-2 border rounded text-lg" type="date" />
          <button onClick={add} className="bg-blue-500 hover:bg-blue-600 transition-all p-3 text-white rounded-lg flex items-center">
            <FaPlus className="mr-2" /> Add Task
          </button>
        </div>
        ...
        <div className="mt-4">
          {todoList.length > 0 ? (
            todoList.map((item) => (
              <TodoItems key={item.id} {...item} deleteTodo={deleteTodo} toggle={toggle} editTodo={editTodo} />
            ))
          ) : (
            <p className="text-center mt-4">No tasks yet!</p>
          )}
